...
//            break;
//        case 101:
//            if([ReportBadDriver CleanTextField:_DescTextview.text].length == 0 || [[ReportBadDriver CleanTextField:_DescTextview.text] isEqualToString:@"Add Description here"]) {
//            _DescTextview.text = @"";
...
//    
//    
//    UIAlertView *FinalCancel = [[UIAlertView alloc] initWithTitle:@"Warning!" message:@"Are You Sure To Cancel This Report?" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
//    [FinalCancel setTag:9933];
//    [FinalCancel show];
//    
//    
////        _isVehicleDescComplete = NO;
...
- (void)viewDidLoad
{
    ...
    ReportBadDriver = [[HelperClass alloc] init];
    [self HideNavigationBar];
    [ReportBadDriver SetViewBackgroundImage:self.view imageName:GLOBALBACKIMAGE];
    [ReportBadDriver setTopView:self.view];
    [ReportBadDriver SetupHeaderViewForReport:self.view viewController:self];
    ...
    _UIScrollViewMain.scrollEnabled = YES;
    _UIScrollViewMain.backgroundColor = [UIColor clearColor];
    [_UIScrollViewMain setContentSize:CGSizeMake(self.view.frame.size.width,self.view.frame.size.height)];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [[picker presentingViewController] dismissViewControllerAnimated:YES completion:^{
        ...
        if (upoloadFirst==223)
        {
            ...
            [self Makeframe:_AddVEhicledescview];
            [_UIScrollViewMain addSubview:_AddVEhicledescview];
            [_UIScrollViewMain setContentSize:CGSizeMake(self.view.frame.size.width,self.view.frame.size.height+90)];//CGRectMake(<#CGFloat x#>, 0, 320, 660);
        }
        ...
        else
        {
            ...
            //[_PriviewScroll setContentSize:CGSizeMake(320, 1200)];
            if(photoChoosen==YES){
                [_PriviewScroll setContentSize:CGSizeMake(320, 900)];
            }
            else{
                [_PriviewScroll setContentSize:CGSizeMake(320, 860)];
            }
...
-(IBAction)FinalSave:(id)sender {
    ...
    if (PreviewData  == FALSE) {
        ...
        if (AccessCustomLocation) {
            ...
            // Accessing custom location provided by user
            NSLog(@"here AccessCustomLocation");
//            ModifiedLocation    = UserDesiredLocation;
             ModifiedLocation    = _locationShow.text;
        }
        
        if (AccessMyLocation) {
            
            // Accessing user device location
            NSLog(@"here AccessMyLocation");
            //modified
